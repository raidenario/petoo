(ns pet-app.api.queries
  "Query API handlers for read operations.
   
   All queries read from denormalized read_model tables for maximum performance.
   Zero JOINs - data is already denormalized by the Projector Worker."
    ...
    (if-not tenant-id
      ...
      (try
        (let [where-clauses (cond-> [:and [:= :a.tenant-id [:cast tenant-id :uuid]]]
              ...
              ;; Query directly from core tables with JOINs
...
(defn get-professional-schedule
  "Get appointments for a professional in a date range.
   ...
   Query params:
     - tenant-id (required)
     - date (required) - YYYY-MM-DD (start date)
  ...
  (let [professional-id (get-in request [:path-params :professional-id])
    ...
    (if (or (not tenant-id) (not date))
      ...
      (try
        ;; Get existing appointments for this professional in the date range
